...
const std::string kUserToken = "sb_publishable_a07aqaPwFEDsx2uwog3yw_CTaEzpoP"; 
...
public:
	...
	void RecordHeartbeatReceived(int32_t frame) {
		...
		{
			auto durationMicroSec = std::chrono::duration_cast<std::chrono::microseconds>(receivedTime - lastHeartbeatSentTime_);
			lastRoundTripTimeMs_ = durationMicroSec.count() / 1000.0;
		}
	}
...
private:
	...
	std::chrono::high_resolution_clock::time_point lastHeartbeatSentTime_; // 最後の送信時刻
	double lastRoundTripTimeMs_ = 0.0; // RTTミリ秒
};
