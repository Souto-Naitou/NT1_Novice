...
int WINAPI WinMain(HINSTANCE, HINSTANCE, LPSTR, int) {
	...
	const std::string supabaseUrl = "wss://oolchvtzizhmniggcaiw.supabase.co/realtime/v1/websocket"
	...
	auto createAndStartWebSocket = [&](RealtimeMonitorState& state) {
		...
		webSocket->setOnMessageCallback([&state, rawWebSocket, &topic, &userToken](const ix::WebSocketMessagePtr& messagePtr) {
			...
			if (messagePtr->type == MessageType::Open) {
				...
				const bool isPhxReply = (messageText.find(R"("event":"phx_reply")") != std::string::npos);

				const bool isOkStatus = (messageText.find(R"("status":"ok")") != std::string::npos) || (messageText.find(R"("response":{"status":"ok")") != std::string::npos);
	...
	while (Novice::ProcessMessage() == 0) {
		...
		const bool canStartConnection =
		    statusSnapshot == RealtimeMonitorState::Status::Idle || statusSnapshot == RealtimeMonitorState::Status::Error || statusSnapshot == RealtimeMonitorState::Status::Closed;
